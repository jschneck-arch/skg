...
from skg_domain_web.ontology import load_attack_paths
from skg_domain_web.policies import load_projection_policy


SOURCE_ID = "projection.web.domain_pack"
TOOLCHAIN = "web"
VERSION = "1.0.0"
...
def _canonical_attack_path_id(attack_path_id: str) -> str:
    policy = load_projection_policy()
    aliases = policy.get("attack_path_aliases") if isinstance(policy, dict) else {}
    if isinstance(aliases, Mapping):
        return str(aliases.get(attack_path_id) or attack_path_id)
    return attack_path_id


def _attack_path_definition(catalog: Mapping[str, Any] | None, attack_path_id: str) -> tuple[str, dict[str, Any]]:
    canonical = _canonical_attack_path_id(attack_path_id)

    if isinstance(catalog, Mapping):
        attack_paths = catalog.get("attack_paths")
        if isinstance(attack_paths, Mapping):
            path = attack_paths.get(canonical)
            if isinstance(path, Mapping):
                return canonical, dict(path)
...
def _priority(status: str) -> int:
    policy = load_projection_policy()
    table = policy.get("status_priority") if isinstance(policy, dict) else {}
...
def compute_web(
    ...
    score_key = str((load_projection_policy().get("score_key") or "web_score"))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Web domain projector")
    ...
    events = _read_events(Path(args.in_file))
    ...
    if not result:
        raise SystemExit(f"Unknown attack_path_id: {args.attack_path_id}")
