...
from skg_domain_host.ontology import load_attack_paths
from skg_domain_host.policies import load_projection_policy


SOURCE_ID = "projection.host.domain_pack"
TOOLCHAIN = "host"
VERSION = "1.0.0"
...
def _latest_wicket_states(events: Iterable[Mapping[str, Any]], required: list[str]) -> dict[str, NodeState]:
    ...
    for wicket_id in required:
        ...
        states[wicket_id] = NodeState(
            ...
            notes=str(payload.get("detail") or payload.get("notes") or ""),
            attributes={
                "status": status,
...
def compute_host(
    ...
    try:
        from skg_services.gravity.state_collapse import load_states_from_events as _collapse
        _raw = _collapse(list(events))
        states = {wid: _raw[wid] for wid in required if wid in _raw}
        for wid in required:
            if wid not in states:
                states[wid] = NodeState.unknown(wid)
    except Exception:
    ...
    score_key = str((load_projection_policy().get("score_key") or "host_score"))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Host domain projector")
