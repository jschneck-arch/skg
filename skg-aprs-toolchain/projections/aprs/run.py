...
def compute_aprs(events, catalog, attack_path_id: str, run_id: str|None=None, workload_id: str|None=None):
    # Determine required wickets
    ap = None
    for item in catalog.get("attack_paths", []):
        if item.get("attack_path_id") == attack_path_id:
            ap = item
            break
    if not ap:
...
def main():
    ...
    p.add_argument("--catalog", default="contracts/catalogs/attack_preconditions_catalog.v1.json")
