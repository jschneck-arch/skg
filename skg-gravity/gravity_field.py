...
INTERP_DIR = Path("/var/lib/skg/interp")
...
def detect_instruments() -> dict:
    ...
    ce_parse_path = Path("/opt/skg/skg-container-escape-toolchain/adapters/container_inspect/parse.py")
...
def load_wicket_states(ip: str) -> dict:
    ...
    Returns {wicket_id: {"status": str, "detail": str, "ts": str, "phi_r": float, "phi_b": float}}
    ...
    for port, scheme in web_ports[:2]:
        ...
        try:
            ...
            if events_file.exists():
                ...
                # Run web projection so interp files feed FeedbackIngester
                # This closes the loop: events → projection → interp → feedback → prior update
                try:
                    proj_script = SKG_HOME / "skg-web-toolchain" / "projections" / "run.py"
                    interp_file = INTERP_DIR / f"web_{ip}_{port}_{run_id[:8]}.json"
                    ...
                    if proj_script.exists():
                        r = subprocess.run(
                            ...
                            cwd=str(SKG_HOME / "skg-web-toolchain" / "projections")
                        )
                        if r.returncode == 0 and interp_file.exists():
                            result["interp_file"] = str(interp_file)
                except Exception:
                    pass  # projection is best-effort; collection already succeeded
    ...
    for port, scheme in web_ports[:1]:
        ...
        try:
            ...
            auth_scan(target=url, out_path=str(events_file),
                      ...
                      password=password,
                      timeout=10.0)
            # Run projection to feed FeedbackIngester
            try:
                proj_script = SKG_HOME / "skg-web-toolchain" / "projections" / "run.py"
                interp_file = INTERP_DIR / f"web_auth_{ip}_{port}_{run_id[:8]}.json"
                INTERP_DIR.mkdir(parents=True, exist_ok=True)
                if proj_script.exists() and events_file.exists():
                    subprocess.run(
                        [sys.executable, str(proj_script),
                         "--in", str(events_file),
                         "--out", str(interp_file),
...
def _exec_container_inspect(ip, target, run_id, out_dir, result):
    ...
    try:
        from parse import (check_running_as_root, check_privileged,
                           check_docker_socket_mounted, check_docker_api_exposed,
                           check_host_network, check_pid_namespace, check_capabilities)
...
def _exec_iot_firmware(ip, target, run_id, out_dir, result):
    ...
    if ssh_target:
        ...
        if image_candidates:
            ...
            try:
                from probe import probe_network_only
...
def gravity_field_cycle(surface_path: str, out_dir: str,
    ...
    for t in landscape:
        ...
        # Launch ALL viable instruments concurrently.
        # The field doesn't take one measurement at a time — different instruments
        # observe different wavelengths simultaneously. nmap, http_collector, and
        # auth_scanner can all run in parallel because they're observing orthogonal
        # conditions. We cap at 4 concurrent instruments to bound resource usage.
        MAX_CONCURRENT = 4
        to_run = candidates[:MAX_CONCURRENT]

        if len(to_run) == 1:
            print(f"    Selected: {to_run[0][1]} (potential={to_run[0][0]:.1f})")
        else:
            print(f"    Selected {len(to_run)} instruments concurrently: " +
                  ", ".join(f"{n}({p:.1f})" for p,n,_ in to_run))
        ...
        def _run_one(args):
            ...
            return _name, execute_instrument(_inst, t["target"], run_id, out_path)
        ...
        best_name = to_run[0][1]
        best_inst = to_run[0][2]
        all_run_insts = [(name, inst) for _, name, inst in to_run]
        ...
        E_after_base = field_entropy(new_states, t["applicable_wickets"], ip=ip)
        ...
        try:
            from skg.kernel.folds import FoldDetector, FoldManager
            new_fd = FoldDetector()
            new_folds = new_fd.detect_all(DISCOVERY_DIR, CVE_DIR, Path("/opt/skg"))
            new_fm = FoldManager()
            for f in new_folds:
        ...
        if result.get("success"):
            ...
            if delta_E > 0:
                ...
                # Genuine failure to reduce entropy — record double entry so
                # failed_to_reduce() fires and gravity shifts next cycle
                for _n, _i in all_run_insts:
                    _i.entropy_history.setdefault(ip, []).append(E_after)
                    _i.entropy_history[ip].append(E_after)
