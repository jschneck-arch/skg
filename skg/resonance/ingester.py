...
ADAPTER_EVIDENCE_SOURCES = {
    ...
    # Host collection
    "ssh_collect": [
        "SSH command execution (id, sudo -l, find -perm -4000)",
        "environment variables and shell history",
        "package manager output (dpkg/rpm/apk/pacman)",
        "kernel version (uname -r)",
        "~/.ssh private key scan",
        "docker/container runtime detection",
        "cloud metadata service probe (IMDS)",
        "process list (ps aux)",
        "domain/Kerberos config (realm, krb5.conf)",
    ],
    "winrm_collect": [
        "WinRM PowerShell command execution",
        "Get-Package / wmic product list",
        "Get-Process (AV/EDR detection)",
        "Get-NetTCPConnection (SMB/RDP exposure)",
        "Win32_ComputerSystem.PartOfDomain",
        "environment variables (Get-ChildItem Env:)",
        "scheduled tasks and service executables",
    ],
    "nmap_scan": [
        "nmap XML output (-oX format)",
        "nmap service version detection (-sV)",
        "nmap NSE script output (vuln scripts)",
        "OS detection (-O)",
    ],
    "msf_session": [
        "Metasploit RPC sessions list (msfrpcd)",
        "MSF database credentials (db.creds)",
        "MSF database loot files (db.loots)",
        "MSF database hosts (db.hosts)",
    ],
}
